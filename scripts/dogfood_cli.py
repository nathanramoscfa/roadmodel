...
PROBES: list[Probe] = [
    ...
    Probe("trivial", "What is the capital of France?", "cheapest model / thinking N/A"),
    Probe(
        "long-context",
    ...
    Probe(
        "creative",
        "Write a short whimsical poem about a lighthouse that falls in love with a passing comet.",
        "top creative tier",
    ),
    Probe("ambiguous-help", "help me with my code", "vacuous — watch tier stability across runs"),
]
...
def run_probe(
    bin_path: str, probe: Probe, run: int, provider: str, engine: dict[str, object], key: str
) -> Result:
    cmd = [
        bin_path,
        "recommend",
    ...
    if proc.returncode != 0:
        return Result(probe.id, run, ok=False, error=(proc.stderr or proc.stdout).strip()[:200])
    try:
...
def score(results: list[Result], tier_map: dict[str, str]) -> list[tuple[str, bool, str]]:
    ...
    checks.append(
        ("B1 no-task-leak", len(leaks) == 0, ", ".join(r.probe_id for r in leaks) or "clean")
    )
    ...
    checks.append(("B7 tier-stable", len(tier_flaky) == 0, ", ".join(tier_flaky) or "tier-stable"))
    checks.append(("B7 exact-model (watch)", True, ", ".join(model_flaky) or "stable"))
    return checks
...
def main() -> int:
    ...
    print(
        f"Dogfooding {len(probes)} prompts x {args.runs} = {total} calls via "
