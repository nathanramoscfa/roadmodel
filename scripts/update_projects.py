...
# Commands this updater once installed under a name it no longer uses. Every
# run removes its own copies of them for every agent; a user's own command of
# the same name is left alone.
RETIRED_COMMANDS = ("roadmodel-update",)  # renamed roadmodel-upgrade in 0.2.45
...
def _sync_antigravity_trust(projects: list[Path], dry_run: bool) -> list[str]:
    ...
    exactly the friction `roadmodel-upgrade` exists to remove when a machine
...
# Keeps the updater's old name: an existing memory file is overwritten only
# when it starts with this exact line.
...
def export_claude_memory(project: Path) -> Optional[str]:
    ...
    parts = [
        ...
        "this file is git-excluded and is regenerated by `roadmodel-upgrade`.",
        "",
    ]
    if index_text:
        parts += [
...
def _git_exclude(project: Path, pattern: str, dry_run: bool) -> bool:
    ...
    if not dry_run:
        ...
        block = "# roadmodel-upgrade: agent memory export (personal, never commit)"
        exclude.write_text(
            (body + "\n\n" if body else "") + block + "\n" + pattern + "\n", encoding="utf-8"
        )
    return True
...
def _agents_md(project: Path) -> str:
    ...
    lines += [
        ...
        "local-only (git-excluded) and regenerated by `roadmodel-upgrade`, so it may be",
        ...
        "## Commands",
        "",
        "`roadmap-project`, `roadmap-phase`, `roadmap-step`, `roadmap-refresh` and",
        "`roadmodel-upgrade` are installed for every agent on this machine (Codex:",
        "`/prompts:roadmap-phase 1`; elsewhere: `/roadmap-phase 1`). See",
        "docs/agent-parity.md in the roadmodel repo.",
        "",
        # The marker keeps the updater's old name: /roadmap-step and
        # /roadmap-refresh recognise the file by this exact line.
...
def _command_files(name: str) -> list[Path]:
    ...
    files = [
        ...
        OPENCODE_DIR / "commands" / f"{name}.md",
    ]
    return files + [d / "prompts" / f"{name}.prompt.md" for d in _vscode_user_dirs()]


def retire_commands(dry_run: bool = False) -> list[str]:
    """Remove this updater's copies of RETIRED_COMMANDS. A copy is ours when it
    carries the updater's filename, which every port of a command body does."""
    report: list[str] = []
    for name in RETIRED_COMMANDS:
        removed = 0
        for path in _command_files(name):
            try:
                ours = path.is_file() and "update_projects.py" in path.read_text(encoding="utf-8")
        ...
        if removed:
            report.append(
                f"{name}: retired — removed {removed} old cop{'y' if removed == 1 else 'ies'}"
            )
    return report
