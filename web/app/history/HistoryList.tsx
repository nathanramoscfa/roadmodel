    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <label className="flex flex-col gap-1 text-sm">
          <span className="font-medium text-brand-slate-700 dark:text-brand-slate-200">Search title</span>
          <input
            type="search"
            value={titleInput}
            ...
            className={
              "rounded-md border border-brand-slate-300 dark:border-brand-slate-700 bg-white dark:bg-brand-slate-800 px-3 py-2 " +
        ...
        <label className="flex flex-col gap-1 text-sm">
          <span className="font-medium text-brand-slate-700 dark:text-brand-slate-200">From</span>
          <input
            type="date"
            value={fromInput}
        ...
        <label className="flex flex-col gap-1 text-sm">
          <span className="font-medium text-brand-slate-700 dark:text-brand-slate-200">To</span>
          <input
            type="date"
            value={toInput}
        ...
        <ul className="space-y-3" data-testid="history-list">
          {filtered.map((c) => (
            <li
              ...
              className={
                "flex flex-col gap-2 rounded-lg border border-brand-slate-200 dark:border-brand-slate-700 " +
                "bg-white dark:bg-brand-slate-800 p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between"
              }
            >
              <div className="min-w-0">
                <p className="truncate font-semibold text-brand-slate-900 dark:text-brand-slate-50">
                  {c.title}
                </p>
                {c.last_message_snippet ? (
                  <p className="mt-1 line-clamp-2 text-sm text-brand-slate-600 dark:text-brand-slate-300">
                    {c.last_message_snippet}
                  </p>
                ) : null}
