"""Decide whether the Gemini thinking refresh (extractor + Opus) needs to run.

Gemini's reasoning facts live on an HTML docs page (``ai.google.dev`` serves no
clean Markdown). Robust change-detection here is deliberately TWO-layered so a
dynamic / flaky devsite can never silently corrupt the tracker:

1. **Validate the fetch.** The declarative ``must_contain_all`` anchors are the
   literal table headers / sentinels that live in the SERVER-rendered HTML. If
   the page comes back as a JS shell, a consent/redirect interstitial, or a
   truncated body, those anchors are absent and validation raises — we do NOT
   try to parse a degenerate page.
2. **Hash the EXTRACTED FACTS, not the raw HTML.** The change signature is the
   ...
   (sorted JSON of the level matrix + budget table + sentinels). Cosmetic
   devsite churn (re-renders, analytics, reordered prose) therefore never
   triggers an expensive Opus run — only a real change to the thinking facts
   does. A structural break (the parse no longer finds the tables) raises
...
- ``docs_changed=true|false`` — same value (single source); emitted distinctly
  so the workflow wiring mirrors the Claude Code / Codex crons.
...
# Reuse the extractor's fetch + parse so the gate and the snapshot agree on
# exactly which facts count. Sibling import (update/ is not a package) — mirror
# check_codex_source.py's sys.path guard.
...
from extract_gemini_thinking import build_snapshot  # noqa: E402, I001

UPDATE_DIR = _UPDATE_DIR
SOURCES_PATH = UPDATE_DIR / "sources-gemini.json"
CACHE_DIR = UPDATE_DIR / ".cache"
DOCS_HASH_PATH = CACHE_DIR / "gemini-docs-hash"
DOCS_HASH_STAGING_PATH = CACHE_DIR / "gemini-docs-hash.new"
...
def validate_docs(text: str, cfg: dict[str, Any]) -> None:
    ...
    headers, so their absence means we did NOT receive the real content
    (JS shell / interstitial / truncation) and must not parse it.
    """
...
def check_source(
    ...
    try:
        ...
        print(
            f"check_gemini_source: {label} fetch/validate/parse error ({exc!r}); "
            "treating as changed",
            file=sys.stderr,
        )
    ...
    print(f"check_gemini_source: {label} {state}")
    return changed
...
def main() -> int:
    ...
    print(f"check_gemini_source: changed={changed} (docs={changed})")
    return 0
