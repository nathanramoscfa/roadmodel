#!/usr/bin/env python3
"""Reconcile the Gemini reasoning blocks of ``docs/model-selector.txt`` with
Google's official Gemini API thinking docs using Opus 4.7.

Gemini has no changelog, so this cron is purely docs-driven: the
deterministically-extracted ``<docs_facts>`` (``update/gemini-thinking.json``)
is the authoritative input, and the change trigger is the extracted-facts hash
(``update/check_gemini_source.py``). Scope is narrow: only the Gemini bullet +
output mapping in ``<thinking-context>`` and the ``google-api`` / ``gemini-cli``
``best-for`` text may be mutated — see ``update/prompt-gemini.md``. The offline
conformance gate (``update/validate_effort_conformance.py`` check E) is the
deterministic backstop that hard-fails a non-conformant Opus edit.

Deliberately isolated from the Claude Code / Codex / Cursor crons per the
blast-radius-isolation principle: they own non-overlapping slices of the
...
PROMPT_PATH = UPDATE_DIR / "prompt-gemini.md"
# The committed docs-facts snapshot (refreshed by extract_gemini_thinking.py
# before this step). Fed to Opus as authoritative for thinking content.
THINKING_JSON_PATH = UPDATE_DIR / "gemini-thinking.json"
LAST_SUMMARY_PATH = UPDATE_DIR / ".last-gemini-summary.txt"
LAST_WARNINGS_PATH = UPDATE_DIR / ".last-gemini-warnings.txt"
...
def read_docs_facts() -> str | None:
    """The committed docs-facts snapshot (update/gemini-thinking.json), or None
...
def parse_result(raw: str) -> dict[str, Any]:
    ...
    Mirrors ``update/update_codex.py::parse_result`` — the longest plausible
...
def main() -> int:
    ...
    if docs_facts is None:
        sys.stderr.write(
            "gemini-thinking.json snapshot missing; run extract_gemini_thinking.py "
            "before this step.\n"
        )
        return 4
    ...
    summary = result.get("summary") or "Reconcile Gemini thinking surface with the docs"
