...
`docs/benchmarks.json` (gap to the category leader: S ≤ 5, A ≤ 20,
B ≤ 35, C ≤ 50 points, else D) for every model AA measures, and the
committed selector is CI-checked against that derivation. NEVER edit those
four attributes on ANY model — an edit is overwritten on a measured model
and, on an unmeasured one, is an editorial call this automation is not
authorized to make. The coding S-tier candidate enumeration in
`<selection-algorithm>` is regenerated by the same script; do not touch it.

A `tier-planning`, `tier-multimodal`, or `tier-speed` rating on an
...
- A leaderboard named in `<benchmark-sources>` is present in this
  ...
  MMMU → `tier-multimodal`; LMArena Elo → `tier-planning`; output
  tokens/sec → `tier-speed`. (Coding / agentic / long-context /
  knowledge sources feed `headline-benchmarks` prose only — those
  letters are derived, see above.)
...
- Coding S-tier guardrail: the enumeration `the candidate set is
  ...
  when the ratings are equivalent for the prompt`. Regenerated
  deterministically by `update/derive_ratings.py --write` after this
  pass — leave it verbatim.
