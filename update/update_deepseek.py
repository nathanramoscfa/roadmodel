#!/usr/bin/env python3
"""Reconcile the DeepSeek reasoning blocks of ``docs/model-selector.txt`` with
DeepSeek's official API thinking-mode docs using Opus 4.7.

DeepSeek has no changelog, so this cron is purely docs-driven: the
deterministically-extracted ``<docs_facts>`` (``update/deepseek-thinking.json``)
is the authoritative input, and the change trigger is the extracted-facts hash
(``update/check_deepseek_source.py``). Scope is narrow: ONLY the DeepSeek bullet
+ output mapping in ``<thinking-context>`` may be mutated — see
``update/prompt-deepseek.md``. (DeepSeek has no first-party ``<method>`` in the
selector today — it is tracked-only, cn-jurisdiction — so unlike the Gemini cron
there is no method ``best-for`` to reconcile.) The offline conformance gate
(``update/validate_effort_conformance.py`` check F) is the deterministic backstop
that hard-fails a non-conformant Opus edit.

Deliberately isolated from the Claude Code / Codex / Gemini / Cursor crons per
the blast-radius-isolation principle: they own non-overlapping slices of the
selector and run on staggered cadences.
"""
...
PROMPT_PATH = UPDATE_DIR / "prompt-deepseek.md"
# The committed docs-facts snapshot (refreshed by extract_deepseek_thinking.py
# before this step). Fed to Opus as authoritative for thinking content.
THINKING_JSON_PATH = UPDATE_DIR / "deepseek-thinking.json"
LAST_SUMMARY_PATH = UPDATE_DIR / ".last-deepseek-summary.txt"
LAST_WARNINGS_PATH = UPDATE_DIR / ".last-deepseek-warnings.txt"
...
def read_docs_facts() -> str | None:
    """The committed docs-facts snapshot (update/deepseek-thinking.json), or None
    if it has not been generated yet."""
    if not THINKING_JSON_PATH.exists():
        return None
    return THINKING_JSON_PATH.read_text()
...
def parse_result(raw: str) -> dict[str, Any]:
    ...
    Mirrors ``update/update_gemini.py::parse_result`` — the longest plausible
    ``roadmodel_txt`` payload wins.
    """
    text = raw.strip()
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Accepted for workflow symmetry with the Claude Code / Codex crons. "
            "This cron is always docs-driven, so reconciliation runs regardless."
        ),
    )
    ...
    if docs_facts is None:
        sys.stderr.write(
            "deepseek-thinking.json snapshot missing; run extract_deepseek_thinking.py "
            "before this step.\n"
        )
        return 4
    ...
    summary = result.get("summary") or "Reconcile DeepSeek thinking surface with the docs"
