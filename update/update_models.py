...
def main() -> int:
    ...
    def run_call(
        ...
        try:
            ...
            sys.stderr.write(f"Opus did not finish the {target!r} pass: {exc}\n")
            return None
        try:
            return parse_result(raw, primary_key=key)
        except json.JSONDecodeError:
            # Bound the dump: the selector payload is ~143KB on one line, which
            # buries the useful part (how the output starts and how it ends) in
            # log noise — and gets dropped by log viewers entirely.
            sys.stderr.write(
                f"Model did not return valid JSON for target={target!r} "
                f"({len(raw)} chars). Head and tail:\n"
            )
