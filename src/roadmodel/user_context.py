...
DEFAULT_USER_CONTEXT_HOME = (
    Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "roadmodel" / "user-context.md"
)
_USER_CONTEXT_TEMPLATE: Final = "user-context.example.md"


def default_user_context_home() -> Path:
    return Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config")) / "roadmodel" / "user-context.md"
