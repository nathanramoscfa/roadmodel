...
_SAAS_HEADER: Final = (
    ...
    "the appended user-context declares 'Consumption headroom: uncapped' for "
    "it (gate condition (c) — the operator does NOT exhaust that pool), AND no "
    "adequate locally-pulled model is declared (gate condition (d) — see LOCAL "
    "MODELS above), out-of-pocket price is FLAT — tiering down "
    ...
    "paths keep the ordinary tier-down behavior below — and so does a "
    "'capped' or undeclared headroom posture (the default) or a usage pool "
    "reported 'tight' / 'exhausted': the gate is CLOSED there, the weekly pool "
    "is a real budget metered by model AND effort, and EFFORT is the "
    "<thinking-context> complexity-ladder value, not the top rung.\n"
...
_SAAS_LADDER_HEADER: Final = (
    ...
    "the platform is subscription-funded, the family is covered, the appended "
    "user-context declares 'Consumption headroom: uncapped' for it (gate "
    "condition (c); a 'capped' or undeclared posture, or a pool reported "
    "'tight' / 'exhausted', keeps the gate CLOSED and the three rules above in "
    "force, with EFFORT at the complexity-ladder value), and no adequate "
    "locally-pulled model is declared (gate condition (d) — see LOCAL MODELS "
    "below), a lower rung SAVES THE USER NOTHING: hold the capability "
