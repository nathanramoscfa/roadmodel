...
<!-- subscription-tiers-reviewed: 2026-07-20 -->
...
reference, since that rate determines how Composer 2.5 calls bill
against the pool — but the `auto` and `premium` model ids no longer
appear as recommendable engines.
...
| ---------------- | ------ | --------- | -------------------------------------------------------------------------------------------------------------------------- |
| (notes refresh) | —      | —         | 2026-07-20 — Cursor's pricing page reworded several rows from "request-based plans" to "legacy request-based plans" and from "Up to 1M tokens in Max Mode" to "Up to 1M tokens with extended context"; Notes columns refreshed verbatim to match |
...
- The jurisdiction code reflects the **operator** — the entity whose
  ...
  Grok 4.5 is listed under Cursor (not xAI) because Cursor operates
  the first-party pool that serves it (jointly trained by Cursor
  and SpaceXAI per Cursor's pricing-page note).
