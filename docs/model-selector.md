...
- Claude (Anthropic API, Claude Code, claude.ai): per-session
  ...
  Opus 5.5, Opus 4.7, Opus 4.8, Sonnet 5, Fable 5, and Fable 5.1
  ...
  (`medium`). Extended thinking can also be toggled with Option+T /
  ...
  2.1.258 is a hotfix for a launch crash on macOS 12. 2.1.259 adds
  `managedMcpServers` policy, `--permission-prompts none` for
  unattended headless hosts, and GitLab MR recognition. 2.1.260
  adds an inline `/diff` panel, prompt-cache-miss diagnostics in
  `/cost`, and a text form of `/advisor` for headless sessions.
  2.1.261 adds `/skill-doctor` for skill context accounting,
  ...
  vocabulary or extended-thinking controls. Claude Code 2.1.280
  ...
  aside). 2.1.280 also stops carrying a saved pre-per-model
  ...
  over an explicit `/effort`, `effortLevel`, project or managed
  setting; the `/effort` vocabulary itself is unchanged.
  2.1.281 stops Alt+T and `/config` from offering a thinking-off
  switch where the model cannot turn thinking off (thinking stays
  on, with a one-line reason). 2.1.282 switches the `ultracode`
  visuals in `/effort` to plain styling. Neither release changes
  the `/effort` vocabulary or the extended-thinking controls.
- OpenAI (Codex, OpenAI API, ChatGPT advanced controls):
  ...
  accepts the same six values. Higher effort spends more reasoning
  tokens before visible output.
...
- Claude Code `/effort`: `low` → `Low`; `medium` → `Medium`;
  ...
  supports effort, Opus 4.7 and Opus 5.5 aside); `xhigh` → `XHigh`
  ...
  Opus 4.8, Sonnet 5, Fable 5, and Fable 5.1. On models that reach
  ...
  `alwaysThinkingEnabled` false) → `Off`.
- OpenAI `reasoning_effort`: `low` → `Low`; `medium` → `Medium`;
  `high` → `High`; `xhigh` / `extra-high` (the "Extra High" tier —
  the UI synonym `extra-high` and the config token `xhigh` are the
  same tier) → `XHigh`; `max` → `Max` (the top step above `xhigh`
  on the Codex `reasoning_effort` scale, on par with the Claude
  `max`-capable models); `ultra` → `Ultracode` (the above-`max` top
  rung on the Codex scale, session-wide multi-agent orchestration
  analogous to Claude Code's `ultracode`).
- Gemini thinking levels: `minimal` → `Off`; `low` → `Low`;
  ...
  same reason it sits BELOW the lowest EFFORT rung: reporting it
  as `Low` would overstate the reasoning the operator actually
  gets. Thinking turned off, on models that allow it (e.g. Gemini
  2.5 Flash-Lite), → `Off`.
...
- Ceiling — `Max`: the top per-call level, used ONLY on models whose
  ...
  Cost (`cheap`) budget posture, never emit `Max` and do not exceed the
...
- **Tier ratings:** Coding **D** · Planning **C** · Agentic **D** · Multimodal **D** · Long-context **D** · Knowledge **D** · Speed **B**
...
- **Supports models:** claude-opus-5-5,claude-opus-5,opus-4.8,claude-fable-5.1,claude-fable-5,opus-4.7,claude-sonnet-5,sonnet-4.6,claude-4.5-haiku
...
- **Supports models:** grok-4.7,grok-4.6,grok-4.3
...
- **Supports models:** claude-opus-5-5,claude-opus-5,opus-4.8,claude-fable-5.1,claude-fable-5,opus-4.7,claude-sonnet-5,sonnet-4.6,claude-4.5-haiku,gpt-5.6-sol,gpt-5.5,gpt-5.6-terra,gpt-5.4,gpt-5.3-codex,gpt-5.2,gpt-5.2-codex,gpt-5.1-codex-max,gpt-5.1-codex,gpt-5.1-codex-mini,gpt-5,gpt-5.6-luna,gpt-5.4-mini,gpt-5.4-nano,gpt-5-mini,gemini-3.1-pro,gemini-3.8-flash,gemini-3.7-flash,gemini-3.6-flash,gemini-3.5-flash,gemini-3-flash,gemini-2.5-flash,grok-4.7,grok-4.6,grok-4.3,deepseek-v4-pro,deepseek-flash,mistral-medium-3.5,mistral-small-4,codestral,glm-5.2,glm-4.6,glm-4.5-air,gpt-oss-120b,gpt-oss-20b,kimi-k3,kimi-k2.7-code,muse-spark-1.3
- **Toggles:** Max Mode — no · Thinking — yes · Orchestration — no
