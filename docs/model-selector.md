...
- DeepSeek (DeepSeek API): a thinking toggle (`enabled` /
  ...
  agentic requests). DeepSeek's docs newly surface `low` as a
  native tier alongside `high` and `max`; there is no `medium`
  tier (for compatibility the API accepts `medium`, mapping it to
...
- DeepSeek: thinking `disabled` → `Off`; `enabled` + effort
  ...
  `max` is the Extra-High top, not the above-`xhigh` cross-
  provider `Max`). DeepSeek effort has no `medium` tier, so no
