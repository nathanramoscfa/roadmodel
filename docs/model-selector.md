...
- Claude (Anthropic API, Claude Code, claude.ai): per-session
  ...
  Opus 4.8, Fable 5). 2.1.268 is a bug-fix / reliability release
  (Claude apps gateway pricing propagation, WebFetch deadline,
  prompt-cache stability under `excludeDynamicSections`, deny-rule
  hardening on symlinked system dirs) with no effort/thinking
  surface changes. The `/effort` vocabulary itself is unchanged.
