...
- Claude (Anthropic API, Claude Code, claude.ai): per-session
  ...
  2.1.271–2.1.274 are reliability / bug-fix / plugin-safety /
  ...
  `claude_code.llm_request` OTel trace span, and assorted fixes) with
  no changes to the `/effort` vocabulary or extended-thinking
  controls. The `/effort` vocabulary itself is unchanged.
