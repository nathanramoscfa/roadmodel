...
- Gemini thinking levels: `minimal` → `Off` (Gemini's documented
  "reasoning off / fast" tier — currently exposed on the 3.x
  Flash / Flash-Lite lines); `low` → `Low`; `medium` → `Medium`;
  `high` → `High` (Gemini tops out at `high` — no `xhigh` or
  `max` tier). Thinking turned off, on models that allow it (e.g.
...
- **Toggles:** Max Mode — no · Thinking — yes
- **Best for:** Programmatic / scripted Gemini use with a Google API key. Pay-per-token at Google's published rates. Powers the roadmodel SaaS free-tier surfaces (/recommend on Gemini 2.5 Flash; /roadmap on Gemini 2.5 Flash with 3.1 Pro escalation). Exposes the Gemini discrete thinking-level dial — `minimal` / `low` / `medium` / `high` on the 3.x models that support it, `low` / `medium` / `high` on the 2.5 generation (per-model coverage varies; e.g. Gemini 3 Pro is low/high only).
...
- **Supports models:** gemini-3.1-pro,gemini-3-pro,gemini-3.5-flash,gemini-3-flash,gemini-2.5-flash
- **Toggles:** Max Mode — no · Thinking — yes
- **Best for:** Terminal-driven Gemini use; the CLI surface for multimodal and long-context Gemini work outside Cursor's pool. Exposes the same Gemini discrete thinking-level dial as the Google API (`minimal` / `low` / `medium` / `high`, per-model).
