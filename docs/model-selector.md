...
- Gemini (Google API, Gemini CLI): a discrete thinking-level
  ...
  and 2.5 model generations (not every model supports every
  level; e.g. Gemini 3 Pro is low/high only, and Gemini 2.5 Pro /
  Flash / Flash-Lite are low/medium/high only with no `minimal`).
...
- Gemini thinking levels: `minimal` → `Off` (Google's `minimal`
  tier is the documented reasoning-off / fastest state on models
  that expose it, e.g. Gemini 3 Flash, Gemini 3.5 Flash, Gemini
  3.6 Flash, Gemini 3.5 Flash-Lite, Gemini 3.1 Flash-Lite Image);
...
- **Toggles:** Max Mode — no · Thinking — yes
- **Best for:** Programmatic / scripted Gemini use with a Google API key. Pay-per-token at Google's published rates. Exposes Gemini's discrete thinking-level dial (`minimal` / `low` / `medium` / `high`; per-model support varies — e.g. Gemini 3 Pro is low/high only, Gemini 2.5 Pro/Flash/Flash-Lite are low/medium/high, 3.x Flash tiers add `minimal`). Powers the roadmodel SaaS free-tier surfaces (/recommend on Gemini 2.5 Flash; /roadmap on Gemini 2.5 Flash with 3.1 Pro escalation).
...
- **Supports models:** gemini-3.1-pro,gemini-3-pro,gemini-3.5-flash,gemini-3-flash,gemini-2.5-flash
- **Toggles:** Max Mode — no · Thinking — yes
- **Best for:** Terminal-driven Gemini use; the CLI surface for multimodal and long-context Gemini work outside Cursor's pool. Exposes Gemini's discrete thinking-level dial (`minimal` / `low` / `medium` / `high`; per-model support varies).
