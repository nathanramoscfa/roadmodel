...
- Gemini (Google API, Gemini CLI): a discrete thinking-level
  ...
  not expose `minimal`). Thinking can be turned off on models that
...
- Gemini thinking levels: `minimal` → `Off` (fast / effectively
  no thinking); `low` → `Low`; `medium` → `Medium`; `high` →
  `High` (Gemini tops out at `high` — no `xhigh` tier). Thinking
  turned off, on models that allow it (e.g. Gemini 2.5
  Flash-Lite), → `Off`.
...
- **Toggles:** Max Mode — no · Thinking — yes
- **Best for:** Programmatic / scripted Gemini use with a Google API key. Pay-per-token at Google's published rates. Exposes the Gemini discrete thinking-level dial (`minimal`, `low`, `medium`, `high`) — not every model supports every level (e.g. Gemini 3 Pro is low/high only; 2.5 generation does not expose `minimal`). Powers the roadmodel SaaS free-tier surfaces (/recommend on Gemini 2.5 Flash; /roadmap on Gemini 2.5 Flash with 3.1 Pro escalation).
...
- **Supports models:** gemini-3.1-pro,gemini-3-pro,gemini-3.5-flash,gemini-3-flash,gemini-2.5-flash
- **Toggles:** Max Mode — no · Thinking — yes
- **Best for:** Terminal-driven Gemini use; the CLI surface for multimodal and long-context Gemini work outside Cursor's pool. Exposes the Gemini discrete thinking-level dial (`minimal`, `low`, `medium`, `high`) subject to each model's supported subset (e.g. Gemini 3 Pro is low/high only; 2.5 generation does not expose `minimal`).
...
Ladder construction rules (the WHOLE POINT of this mode — obey strictly):
  ...
  6. Each pick independently obeys every algorithm rule (availability,
     ...
     funding, thinking/max-mode/orchestration bmapping).
