...
- Claude (Anthropic API, Claude Code, claude.ai): per-session
  ...
  2.1.271–2.1.272 are reliability / bug-fix releases (Remote
  cloud/self-hosted fast mode, `/config` mouse support, per-command
  Bash `allowed_domains`, and assorted fixes) with no changes to
  the `/effort` vocabulary or extended-thinking controls.
