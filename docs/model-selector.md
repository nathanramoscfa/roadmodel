...
`.
  ...
  <availability-context>
    ...
    Cold-start fallback — treat the models listed here as unavailable ONLY when
    no runtime override is present (enforced at Step 0a of
    `<selection-algorithm>`). This list is currently EMPTY: no catalogued model
    is under a standing provider access restriction, so at cold-start every
    catalogued model is treated as available. (Fable 5's 2026-06-12
    export-control restriction was lifted and confirmed by grounded AI
    verification on 2026-07-02, removing it from both the runtime override and
    this fallback.) If a provider later restricts a model, add a bullet here —
    `- \`model-id\` (Name) — reason` — so the fail-closed default benches it
    before the daily probe catches up.
