...
- **Pricing:** Input $3.00/M · Output $15.00/M
- **Tier ratings:** Coding **A** · Planning **A** · Agentic **S** · Multimodal **A** · Long-context **A** · Knowledge **A** · Speed **B**
- **Headline benchmarks:** AA Intelligence Index 53.4 (Adaptive Reasoning, Max Effort); τ²-bench retail pass_1 (Sonnet 4.6 lineage baseline pending own submission); Anthropic-native 1M-token context on Claude Code default; LMArena WebDev Elo 1551.3 (thinking, #6)
...
- **Best for:** Anthropic's Sonnet 4.6 successor at the same high-tier pricing — placeholder tier ratings inherited from sonnet-4.6 pending τ²-bench and full editorial pass; carries native 1M-token context by default on Claude Code (with a `CLAUDE_CODE_DISABLE_1M_CONTEXT` cap for controlled deployments) and exposes the full `/effort` dial through `max`, making it the sonnet-class pick for long-context agentic coding under Anthropic subscriptions; note the launch-promo $2/$10 input/output pricing through August 31, 2026 (post-promo returns to the $3/$15 High-tier rates catalogued here)
...
- **Headline benchmarks:** AA Intelligence Index 41.9; AA Coding Index 60.8; τ²-bench retail pass_1 90.1; released June 12, 2026
- **Pricing notes:** Hidden by default
- **Best for:** Moonshot's Kimi K2.7 Code — a cn-jurisdiction, coding-focused agentic MoE (1T total / 32B active) at $4.00/M output that carries AA Intelligence Index 41.9 and AA Coding Index 60.8; the successor to Kimi K2.5 in Cursor's Moonshot pool, positioned as a cost-conscious cross-provider option when the cn jurisdiction is acceptable and vendor diversity is desired (routed via Cursor's pool only — no direct Moonshot access method is currently enumerated in the access-methods block)
...
- **Supports models:** opus-4.8,claude-fable-5,opus-4.7,gpt-5.5,claude-sonnet-5,sonnet-4.6,gpt-5.4,gpt-5.3-codex,gpt-5.2,gemini-3.1-pro,gemini-3-pro,gpt-5,gpt-5.1-codex,grok-4.3,claude-4.5-haiku,gpt-5.4-mini,gpt-5.4-nano,composer-2,composer-2.5,gemini-2.5-flash,gemini-3-flash,gemini-3.5-flash,gpt-5-mini,kimi-k2.7-code
  ...
  <selection-algorithm>
    ...
    Guardrails:
      ...
      - For PRIMARY = `multimodal`, only consider models with tier-multimodal
        of S or A (currently: gemini-3-flash, gemini-3-pro, gemini-3.1-pro at S; sonnet-4.6, gpt-5.4, claude-sonnet-5, opus-4.7, opus-4.8, gpt-5.5 at A).
