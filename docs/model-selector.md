...
- **Best for:** Anthropic's new top-of-line Fable family flagship (no predecessor) — S-tier across coding, planning, agentic, multimodal, long-context, and knowledge, leading HLE (55.5%) and Terminal-Bench Hard (62.9) with state-of-the-art vision and a 1M default context; about 2x the cost of Opus 5 and latency-slow (output ~64 tokens/s), so reserve for the hardest reasoning, agentic, and vision work where maximum capability outweighs cost and speed; security-guardrail trips auto-route to Opus. Tier profile sourced from the catalog cron's 2026-06-11 dry-run reconciliation against the live benchmark sources (τ²-bench retail not yet published for this model), pending editorial confirmation in the next refresh.
...
- **Headline benchmarks:** AA Intelligence Index 60.9 (max) / 59.0 (xhigh); HLE 49.5% (max); Terminal-Bench 2.1 88.0; τ²-bench banking pass_1 44.3%
...
- **Headline benchmarks:** AA Intelligence Index 56.6 (max); HLE 42.9%; Terminal-Bench 2.1 88.0; Output Speed 108.7 tokens/s
...
- **Best for:** OpenAI's mid-tier GPT-5.6 variant between Sol (flagship) and Luna (mini) at the same high-tier pricing as GPT-5.4 ($15/M output) — Artificial Analysis Intelligence Index 56.6 (max) matches near-frontier reasoning while output speed (~109 tokens/s) is faster than most peers; pick for balanced near-frontier reasoning, agentic execution, and speed when Sol's very-high tier isn't justified but stronger throughput than GPT-5.4 is desired.
...
- **Headline benchmarks:** AA Intelligence Index 52.0 (high reasoning); τ²-bench banking pass_1 32.2%; Output Speed 240.8 tokens/s
- **Pricing notes:** Hidden by default
...
- **Headline benchmarks:** AA Intelligence Index 60.9 (high); HLE 42.9%; Terminal-Bench 2.1 88.4; τ²-bench banking pass_1 50.7%
- **Pricing notes:** Jointly trained by Cursor and SpaceXAI; 50% launch discount for one week starting August 12, 2026
- **Best for:** SpaceXAI's Grok 4.6 flagship (released August 12, 2026), jointly trained by Cursor and SpaceXAI, at the same $6/M output price as Grok 4.5 — placeholder tier ratings inherited from grok-4.5 with benchmark-grounded confirmation from AA Intelligence Index (60.9 high), Terminal-Bench 2.1 (88.4), and τ²-bench banking (50.7% pass_1) placing it among the strongest cheap-tier reasoning models; carries the Grok lineage's signature very-large-context capability. Supersedes grok-4.5 in the same series per the equal-output-price replacement rule.

#### GPT-5.6 Luna — `gpt-5.6-luna`

- **Pricing:** Input $1.00/M · Output $6.00/M
- **Tier ratings:** Coding **B** · Planning **A** · Agentic **A** · Multimodal **B** · Long-context **B** · Knowledge **A** · Speed **S**
- **Headline benchmarks:** AA Intelligence Index 52.3 (max) / 50.1 (xhigh); HLE 39.5% (max); Output Speed 159.5 tokens/s (max) / 146.7 (xhigh)
...
- **Best for:** OpenAI's smallest GPT-5.6 variant at low-tier pricing ($6/M output) — strong AA Intelligence Index (52.3 max) for the price and very high output throughput (~160 tokens/s), positioning it as a cost-efficient near-frontier alternative to Gemini 3.5 Flash on the same tier when GPT-family behavior is preferred; pick when latency and cost dominate but broader reasoning quality than smaller mini/nano variants is required.
...
- **Headline benchmarks:** AA Intelligence Index 29.9; Output Speed 120.4 tokens/s; AA-Omniscience -4.2; latency leader among Claude family
  ...
  <model-options>
    ...
    <tier cost="low">
      ...
      <model id="gpt-5.6-luna" name="GPT-5.6 Luna"
             input-price-per-1m="$1.00" output-price-per-1m="$6.00"
             jurisdiction="us"
             tier-coding="B" tier-planning="A" tier-agentic="A"
             tier-multimodal="B" tier-long-context="B" tier-knowledge="A"
             tier-speed="S"
