...
- OpenAI (Codex, OpenAI API, ChatGPT advanced controls):
  ...
  `none` (plan-mode-only, off). GPT-5.6 (Sol / Terra / Luna)
  additionally supports a `max` step above `xhigh` per OpenAI's
  documented reasoning-effort ladder (none/low/medium/high/xhigh/max).
...
- OpenAI `minimal` → `Off`; `low` → `Low`; `medium` → `Medium`;
  ...
  `max` (GPT-5.6 Sol / Terra / Luna only) → `Max`. On non-5.6 GPT
  models OpenAI's scale tops out at `xhigh`, so no such call
  maps to `Max`; the `Max` slot for those models remains reserved
  for the Claude `max`-capable set.
...
- Ceiling — `Max`: the top output level, used ONLY on models whose
  ...
  Opus 4.7, Opus 4.8, Sonnet 5, Fable 5, and — per OpenAI's
  GPT-5.6 documented ladder — GPT-5.6 Sol, GPT-5.6 Terra,
  GPT-5.6 Luna). On such a model, prefer `Max` over `XHigh` when the
  appended user-context declares a Quality (`best`) budget posture,
  or for the most demanding tasks (the `XHigh` conditions above
  taken to their limit). Under a Cost (`cheap`) budget posture,
  never emit `Max` and do not exceed the lowest level that clears
  the task. Absent any budget posture, `XHigh` stays the default
  ceiling and `Max` is reserved for the genuinely hardest tasks on
  max-capable models. Never emit `Max` for a model whose row lacks
  a `max`-above-`xhigh` step.
...
- **Pricing:** Input $5.00/M · Output $25.00/M
- **Tier ratings:** Coding **S** · Planning **S** · Agentic **S** · Multimodal **A** · Long-context **S** · Knowledge **S** · Speed **D**
- **Headline benchmarks:** AA Intelligence Index 60.7 (max effort); HLE 52.6% (max); Terminal-Bench 2.1 89.1; τ²-bench banking pass_1 30.3%
...
- **Best for:** Anthropic's Opus 4.8 successor at the same very-high tier pricing — placeholder tier ratings inherited from opus-4.8 (Opus 5 leads AA Intelligence Index at 60.7 max effort, HLE 52.6%, and Terminal-Bench 2.1 89.1, and Anthropic positions it as near-Fable capability at Opus speed and cost); native 1M context; pick when top-of-line Anthropic capability is required and Fable 5's ~2x cost is not justified for the workload.
...
- **Best for:** Anthropic's new top-of-line Fable family flagship (no predecessor) — S-tier across coding, planning, agentic, multimodal, long-context, and knowledge, leading HLE (53.3%) and Terminal-Bench Hard (62.9) with state-of-the-art vision and a 1M default context; about 2x the cost of Opus 5 and latency-slow (output ~71 tokens/s), so reserve for the hardest reasoning, agentic, and vision work where maximum capability outweighs cost and speed; security-guardrail trips auto-route to Opus.
...
- **Headline benchmarks:** AA Intelligence Index 55.0 (max); HLE 41.8%; Terminal-Bench 2.1 88.0; Output Speed 165.4 tokens/s (max)
...
- **Best for:** OpenAI's mid-tier GPT-5.6 variant between Sol (flagship) and Luna (mini) at the same high-tier pricing as GPT-5.4 ($15/M output) — Artificial Analysis Intelligence Index 55.0 (max) matches near-frontier reasoning while output speed (~165 tokens/s) is faster than most peers; pick for balanced near-frontier reasoning, agentic execution, and speed when Sol's very-high tier isn't justified but stronger throughput than GPT-5.4 is desired.

#### Kimi K3 — `kimi-k3`

- **Pricing:** Input $3.00/M · Output $15.00/M
- **Tier ratings:** Coding **S** · Planning **A** · Agentic **S** · Multimodal **D** · Long-context **S** · Knowledge **A** · Speed **C**
- **Headline benchmarks:** AA Intelligence Index 57.1; HLE 44.3%; Terminal-Bench 2.1 85.0; τ²-bench banking pass_1 33.4%; 1M-token context
...
- **Best for:** Moonshot's Kimi K3 — the first Moonshot High-tier entry, a text-only cn-jurisdiction reasoning / coding model with a 1M-token context window and Artificial Analysis Intelligence Index 57.1 that puts it in the near-frontier band alongside Opus 5 and GPT-5.6 Sol at a fraction of the very-high-tier output price; routed via Cursor's pool only, cn-jurisdiction excluded by the default allowed-jurisdictions list unless the user opts into cn.
...
- **Headline benchmarks:** AA Intelligence Index 50.2 (high reasoning); τ²-bench retail pass_1 45.6 (Gemini 3.5 Flash); Output Speed 265.6 tokens/s
- **Pricing notes:** Hidden by default
...
- **Tier ratings:** Coding **B** · Planning **A** · Agentic **A** · Multimodal **B** · Long-context **B** · Knowledge **A** · Speed **S**
- **Headline benchmarks:** AA Intelligence Index 50.1 (high); HLE 38.3%; Output Speed 255.3 tokens/s; τ²-bench banking pass_1 24.5%
- **Pricing notes:** -
- **Best for:** Google's Gemini 3.5 Flash successor at the same input price and a lower output price ($7.50/M vs $9.00/M) — placeholder tier ratings inherited from gemini-3.5-flash pending editorial refresh; AA Intelligence Index 50.1 (high) roughly matches its predecessor while the reduced output price makes it the default cheap-tier Gemini Flash pick going forward.

#### GPT-5.6 Luna — `gpt-5.6-luna`

- **Pricing:** Input $1.00/M · Output $6.00/M
- **Tier ratings:** Coding **B** · Planning **A** · Agentic **A** · Multimodal **B** · Long-context **B** · Knowledge **A** · Speed **S**
- **Headline benchmarks:** AA Intelligence Index 51.2 (max) / 49.1 (xhigh); HLE 37.2% (max); Output Speed 220.4 tokens/s (max) / 208.9 (xhigh)
...
- **Best for:** OpenAI's smallest GPT-5.6 variant at low-tier pricing ($6/M output) — strong AA Intelligence Index (51.2 max) for the price and very high output throughput (~220 tokens/s), positioning it as a cost-efficient near-frontier alternative to Gemini 3.5 Flash on the same tier when GPT-family behavior is preferred; pick when latency and cost dominate but broader reasoning quality than smaller mini/nano variants is required.
...
- **Headline benchmarks:** AA Intelligence Index 53.8 (high); Terminal-Bench 2.1 81.6; τ²-bench banking 32.6%; LMArena WebDev Elo 1557
...
- **Headline benchmarks:** AA Intelligence Index 29.6; Output Speed 149.2 tokens/s; AA-Omniscience -4.2; latency leader among Claude family
...
- **Headline benchmarks:** AA Intelligence Index 41.9; τ²-bench retail pass_1 90.1%; LMArena WebDev Elo 1473
- **Pricing notes:** Hidden by default
  ...
  <selection-algorithm>
    ...
    Guardrails:
      ...
      - For PRIMARY = `coding` at S-tier requirement, the candidate set is
        gpt-5.1-codex, gpt-5.1-codex-max, gpt-5.3-codex, gpt-5.2-codex, kimi-k3, opus-4.7, opus-4.8, claude-opus-5, gpt-5.5, gpt-5.6-sol; cost tie-breaker favors
