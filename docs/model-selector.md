...
- **Pricing:** Input $5.00/M · Output $25.00/M
- **Tier ratings:** Coding **S** · Planning **S** · Agentic **S** · Multimodal **A** · Long-context **S** · Knowledge **S** · Speed **D**
- **Headline benchmarks:** AA Intelligence Index 60.7 (max, #1 among Opus lineage); HLE 52.6%; Terminal-Bench 2.1 89.1%; τ²-bench retail pass_1 89.1%
...
- **Best for:** Anthropic's Claude Opus 5 — the new default Opus in Claude Code 2.1.219+ at the same very-high tier pricing as Opus 4.7/4.8 ($5/$25), leading the Opus lineage on AA Intelligence Index (60.7 max) with HLE 52.6% and Terminal-Bench 2.1 89.1%; placeholder tier ratings inherited from opus-4.8 pending independent capability review, pick for the most demanding reasoning, coding, and agentic work when the newest Anthropic frontier model is preferred over the earlier Opus generations.
...
- **Best for:** Anthropic's new top-of-line Fable family flagship (no predecessor) — S-tier across coding, planning, agentic, multimodal, long-context, and knowledge, leading HLE (53.3%) and Terminal-Bench Hard (62.9) with state-of-the-art vision and a 1M default context; about 2x the cost of Opus 5 and latency-slow (output ~69 tokens/s), so reserve for the hardest reasoning, agentic, and vision work where maximum capability outweighs cost and speed; security-guardrail trips auto-route to Opus.
...
- **Headline benchmarks:** AA Intelligence Index 55.0 (max); HLE 41.8%; Terminal-Bench 2.1 88.0; Output Speed 173.6 tokens/s
...
- **Best for:** OpenAI's mid-tier GPT-5.6 variant between Sol (flagship) and Luna (mini) at the same high-tier pricing as GPT-5.4 ($15/M output) — Artificial Analysis Intelligence Index 55.0 (max) matches near-frontier reasoning while output speed (~174 tokens/s) is faster than most peers; pick for balanced near-frontier reasoning, agentic execution, and speed when Sol's very-high tier isn't justified but stronger throughput than GPT-5.4 is desired.

#### Kimi K3 — `kimi-k3`

- **Pricing:** Input $3.00/M · Output $15.00/M
- **Tier ratings:** Coding **A** · Planning **A** · Agentic **A** · Multimodal **D** · Long-context **S** · Knowledge **A** · Speed **C**
- **Headline benchmarks:** AA Intelligence Index 57.1; HLE 44.3%; Terminal-Bench 2.1 85.0%; 2.8T-parameter MoE with 1M-token context and always-on thinking
...
- **Best for:** Moonshot's flagship Kimi K3 (2.8T MoE, 1M context, always-on thinking) at Sonnet-class pricing ($3/$15) — Artificial Analysis Intelligence Index 57.1 with HLE 44.3% and Terminal-Bench 2.1 85.0% places it near the US frontier for reasoning and agentic coding at a lower cost, with signature very-large-context strength. Text-only; cn-jurisdiction (excluded by the default allowed-jurisdictions list unless the user opts into cn); notably slow (~32 tokens/s) so not for latency-sensitive work. Routed via Cursor's pool with no separate cache-write fee.
...
- **Tier ratings:** Coding **B** · Planning **A** · Agentic **A** · Multimodal **B** · Long-context **B** · Knowledge **A** · Speed **S**
- **Headline benchmarks:** AA Intelligence Index 50.1 (high); HLE 38.3%; LMArena WebDev Elo 1527.8; Output Speed 234.8 tokens/s
- **Pricing notes:** -
- **Best for:** Google's Gemini 3.6 Flash successor to 3.5 Flash at 17% cheaper output ($1.50/$7.50) — placeholder tier ratings inherited from gemini-3.5-flash pending independent capability review; matches 3.5 Flash on AA Intelligence Index (50.1 high) with slightly higher throughput (~235 tokens/s) and native multimodal. Prefer gemini-3.6-flash over gemini-3.5-flash when both are available since 3.6 supersedes 3.5 within the same series per the lower-output-price replacement rule.

#### GPT-5.6 Luna — `gpt-5.6-luna`

- **Pricing:** Input $1.00/M · Output $6.00/M
- **Tier ratings:** Coding **B** · Planning **A** · Agentic **A** · Multimodal **B** · Long-context **B** · Knowledge **A** · Speed **S**
- **Headline benchmarks:** AA Intelligence Index 51.2 (max) / 49.1 (xhigh); HLE 37.2% (max); Output Speed 264.4 tokens/s (max) / 209.4 (xhigh)
...
- **Best for:** OpenAI's smallest GPT-5.6 variant at low-tier pricing ($6/M output) — strong AA Intelligence Index (51.2 max) for the price and very high output throughput (~264 tokens/s), positioning it as a cost-efficient near-frontier alternative to Gemini 3.5 Flash on the same tier when GPT-family behavior is preferred; pick when latency and cost dominate but broader reasoning quality than smaller mini/nano variants is required.
...
RATIONALE: [Emit THREE labelled segments in this EXACT order and format, so
            ...
            sentence, no restating the prompt:
              ...
              justification of the settings, NEVER funding, how-to-run, or
              setup instructions>
            Each label is upper-case followed by a segment; keep
            every segment short and justifying the pick only, and NEVER
            perform the task.]
